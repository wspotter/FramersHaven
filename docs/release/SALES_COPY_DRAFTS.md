...
FramersHaven is for independent framing shops and studio operators who want one local place to manage intake, design, quoting, production, and customer handoff without relying on a hosted service.

It is not a POS replacement, an accounting platform, or a vendor catalog service. It does not send messages, process payments, or sync records to external services.
...
- Local catalog package imports with preview support
- Local CSV export for accounting review
- Windows-ready ZIP/folder workflow
- Optional paid setup/support call

## Data Safety Note

FramersHaven is local-first. Operator-supplied catalog files, customer records, exports, backups, and preview images stay on the workstation or trusted private LAN unless the operator moves them manually.

The app does not include vendor catalogs, customer records, accounting credentials, or online billing. Accounting CSV export is a local handoff format only and does not connect to an accounting service. No accounting credentials or API tokens are included.

## Limitations

- Messaging is manual only; the app prepares email and SMS text but does not send messages.
- Payment handling remains outside this app.
- Accounting support is local CSV export only, not API sync.
- The current multi-opening flow supports a basic two-opening layout.
- Workstation Edition is Windows-focused; other platforms are community-supported.
- This is a local-first app, not a hosted service. Do not expose it directly to the public internet.
